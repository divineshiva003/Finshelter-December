...
const getDashboardData = async (req, res) => {
	try {
		// Get all services with lean for better performance
		const services = await Service.find({}).lean();

		// Get all users with populated service details and complete information
		// Use lean() for better performance and select to exclude sensitive fields
		const users = await User.find({})
		...
		// Transform user data - lean() returns plain objects, no need for toObject()
		const transformedUsers = users.map((user) => ({
			...user,
			services: user.services ? user.services.map((service) => ({
				...service,
				name: service.serviceId ? service.serviceId.name : "Unknown Service",
				price: service.serviceId ? service.serviceId.price : 0,
			})) : [],
